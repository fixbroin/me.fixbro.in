...
- A domain name (e.g. `wecanfix.in`) pointed to your VPS IP address.
...
CREATE DATABASE wecanfix_db CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- 2. Create MySQL User with a Strong Password
CREATE USER 'wecanfix_user'@'localhost' IDENTIFIED BY 'SuperStrongPassword123!';

-- 3. Grant full permissions to the user on wecanfix_db
GRANT ALL PRIVILEGES ON wecanfix_db.* TO 'wecanfix_user'@'localhost';
...
git clone <YOUR_GIT_REPOSITORY_URL> wecanfix
cd wecanfix
...
pm2 start .next/standalone/server.js --name "wecanfix-web"
...
sudo nano /etc/nginx/sites-available/wecanfix
```
...
sudo ln -s /etc/nginx/sites-available/wecanfix /etc/nginx/sites-enabled/
sudo nginx -t
...
3. Check status commands:
   ...
   - Check PM2 logs: `pm2 logs wecanfix-web`
